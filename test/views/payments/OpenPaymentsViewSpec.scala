...
class OpenPaymentsViewSpec extends ViewBaseSpec {
  ...
  object Selectors {
    ...
    private val columnOne: Int => String = row => s"#payment-$row > table > tbody > tr > td"
    val title: Int => String = row => s"${columnOne(row)} > h2"
    val description: Int => String = row => s"${columnOne(row)} > h2 > div"
    val due: Int => String = row =>s"${columnOne(row)} > div:nth-child(2)"

    private val columnTwo: Int => String = row => s"#payment-$row > table > tbody > tr > td:nth-child(2)"
    val amount: Int => String = row => s"${columnTwo(row)} > span"

    private val columnThree: Int => String = row => s"#payment-$row > table > tbody > tr > td:nth-child(3)"
