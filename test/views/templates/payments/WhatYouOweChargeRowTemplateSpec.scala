...
class WhatYouOweChargeRowTemplateSpec extends ViewBaseSpec {
  ...
  object Selectors {
    val title = "h2"
    val due = "td > div.govuk-hint"

    val amount = "tr:nth-child(1) > td:nth-child(2) > span"
    val amountData = "tr:nth-child(1) > td:nth-child(2) > span[data-amount]"

    val payLink = s"tr > td:nth-child(3) > a"
    val payText = s"$payLink > span"
    val directDebitText = s"tr > td:nth-child(3) > p > a > span:nth-of-type(2)"
    val payHiddenContent = s"$payLink > span:nth-of-type(2)"
    val viewReturnLink = s"tr > td:nth-child(3) > p > a"
